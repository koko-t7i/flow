...
def render_markdown(snapshot: dict[str, object]) -> str:
    ...
    lines.extend(
        [
            ...
            "- If SSH fails because of publickey or passphrase state, use the configured ignored passphrase file for git retry when present; otherwise use authenticated `gh`/`glab` for remote API operations and token-backed HTTPS fallback for git operations.",
